...
int main(int argc, char **argv) {
    ...
    //some "magic numbers"
    double sample_dt = 0.005; //specify a sample period of 10ms  
    ...
    twist_cmd.angular.z=0.0; //and stop spinning in place (to the left)
    ...
    twist_cmd.angular.z=-yaw_rate; //and start spinning in place (to the right)
    timer=0.0; //reset the timer
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
	...
	twist_cmd.angular.z=yaw_rate; //and start spinning in place (to the left)
    timer=0.0; //reset the timer
    while(timer<3.25) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
